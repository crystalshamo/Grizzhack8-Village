...
app.post("/api/posts/:id/like", async (req, res) => {
 try {
   await db.query("UPDATE Posts SET like_count = like_count + 1 WHERE post_id = ?", [req.params.id]);
   const [[post]] = await db.query("SELECT like_count FROM Posts WHERE post_id = ?", [req.params.id]);
...
app.post("/api/posts/:id/unlike", async (req, res) => {
 try {
   await db.query("UPDATE Posts SET like_count = GREATEST(like_count - 1, 0) WHERE post_id = ?", [req.params.id]);
   const [[post]] = await db.query("SELECT like_count FROM Posts WHERE post_id = ?", [req.params.id]);
   res.json({ like_count: post.like_count });
 } catch (err) {
   res.status(500).json({ error: err.message });
...
// GET all chats for a user (with last message, member count, etc.)
app.get("/api/chats/:user_id", async (req, res) => {
  const { user_id } = req.params;

  try {
    console.log(`📥 GET /api/chats/${user_id}`);

    // Get all chats the user is a member of
    const [chats] = await db.query(
      `SELECT DISTINCT c.chat_id, c.is_group, c.created_at
       FROM Chats c
       JOIN ChatMembers cm ON c.chat_id = cm.chat_id
       WHERE cm.user_id = ?
       ORDER BY c.created_at DESC`,
    ...
    if (chats.length === 0) {
      console.log(`✅ User ${user_id} has 0 chats`);
      return res.json([]);
    }

    // Enrich each chat with last message, member count, names
    const enrichedChats = await Promise.all(
      chats.map(async (chat) => {
        // Get last message
        const [messages] = await db.query(
          `SELECT m.message_id, m.content, m.created_at, u.name as sender_name
           FROM Messages m
        ...
        if (!chat.is_group && members.length >= 2) {
          // For 1-to-1 chats, show the other person's name
          const otherMember = members.find(m => m.user_id != user_id);
          chatName = otherMember ? otherMember.name : `Chat ${chat.chat_id}`;
        } else if (chat.is_group && members.length > 0) {
          // For group chats, show comma-separated names (limit to 2 for space)
          const names = members.map(m => m.name).slice(0, 2).join(', ');
          chatName = members.length > 2 
            ? `${names} +${members.length - 2}` 
            : names;
        }

        // Format time (e.g., "2:30 PM" or "Jan 15")
        let lastTime = '';
        if (lastMessage) {
          ...
          if (isToday) {
            lastTime = msgDate.toLocaleTimeString('en-US', { 
              hour: '2-digit', 
              minute: '2-digit',
              hour12: true 
            });
          } else {
            lastTime = msgDate.toLocaleDateString('en-US', {
              month: 'short',
              day: 'numeric'
            });
          }
    ...
    console.log(`✅ Returning ${enrichedChats.length} chats for user ${user_id}`);
...
app.get("/api/chats/:chat_id/members", async (req, res) => {
...
app.post("/api/chats/:chat_id/messages", async (req, res) => {
  ...
  try {
    console.log(`📤 POST /api/chats/${chat_id}/messages from user ${sender_id}`);
...
app.post("/api/donations/:id/contribute", async (req, res) => {
  ...
  try {
    ...
    if (contributor_id && donation && String(contributor_id) !== String(donation.user_id)) {
      const [[contributor]] = await db.query(
        "SELECT name FROM Users WHERE user_id = ?", [contributor_id]
      );
      await db.query(
        "INSERT INTO Notifications (user_id, message, type) VALUES (?, ?, ?)",
        [
          donation.user_id,
          `${contributor?.name ?? "Someone"} can contribute ${amount} to your request "${donation.title || donation.item}". Start a chat to coordinate the exchange!`,
          `contribution:${contributor_id}`,
        ]
      );
    }

    res.json({ message: "Contribution recorded", donation });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.put("/api/events/:id", async (req, res) => {
  ...
  try {
    await db.query(
      "UPDATE Events SET title = ?, description = ?, event_date = ?, location = ? WHERE event_id = ?",
