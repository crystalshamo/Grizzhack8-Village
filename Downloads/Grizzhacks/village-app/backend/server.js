...
app.post("/api/chats", async (req, res) => {
 ...
 try {
   const [result] = await db.query("INSERT INTO Chats (is_group) VALUES (?)", [is_group ?? false]);
 ...
 try {
   const [results] = await db.query(
     `SELECT m.*, u.name AS matched_user_name FROM Matches m
      JOIN Users u ON (CASE WHEN m.user1_id = ? THEN m.user2_id ELSE m.user1_id END = u.user_id)
